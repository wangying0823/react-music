...
class PlayBar extends Component {
	constructor(props){
      ...
      currModeIndex: 0, //当前模式
      showTip: false, // 展示tip,
      showVloumeTab: false, // 是否显示音量tab
      volume: 50, // 音量
		}
		this.play = this.play.bind(this);
		this.pause = this.pause.bind(this);
		this.toggleVolumeTab = this.toggleVolumeTab.bind(this);
	...
	componentDidMount(){
		...
		if(localStorage.currMusic){
			...
			if(currMusic.info){
				...
				this.props.dispatch(changeCurrMusic(currMusic.info,id,false))
			}
		}
		// 歌曲准备播放时
    ...
    // 当前歌曲结束时
    this.player.onended = () => {
      if (this.state.currModeIndex === 0) { // 循环
        this.chooseSong('next');
      } else if (this.state.currModeIndex === 2) { // 随机
        this.chooseSong('random');
      } else if (this.state.currModeIndex === 1) { // 单曲
        this.player.load();
      }
    };
	}
  ...
  changePlayTime(value){
    ...
    // 播放时间 播放进度 当前播放index  循环list 音量tab
...
class ListTab extends Component{
  ...
  componentWillReceiveProps(np) {
    ...
    const lrclist = currMusic.lrc; //歌词
    let playListCon = null; // 音乐list内容
    let blurPicUrl = null; // 当前音乐url
    if (currMusic.info) {
