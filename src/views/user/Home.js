...
class Home extends Component{
    ...
    api.getUserInfo(id).then(res => { // 获取用户信息
      if (res.data.code === 200) {
        this.setState({
          userInfo: res.data,
        });
    ...
    api.getUserDj(id).then(res => { // 获取dj
      if (res.data.code === 200) {
        this.setState({
    ...
    api.getUserRecord(id).then(res => { //获取听歌记录
      if (res.data.code === 200) {
        this.setState({
          records: res.data.allData.slice(0, 10) || [],
          loadingRecord: false,
    ...
    api.getUserPlaylist(id).then(res => { // 获取用户歌曲
      if (res.data.code === 200) {
        this.setState({
                ...
                spl.length ? spl.map((i, index) => (
                  <li key={index}>
                    <div className="u-cover u-cover-1">
                      ...
                      <Link to={`/playlist?id=${i.id}`} className="msk" title={i.name}/>
                      <div className="bottom">
                        <a className="icon-play f-fr" href="javascript:;" title="播放" onClick={() => this.changePlaylist(i.id)}/>
                        <span className="icon-headset" />
                        <span className="nb">{numberFormat(i.playCount)}</span>
                      </div>
                    </div>
