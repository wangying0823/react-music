...
class Album extends Component{
  ...
  render(){
  	const { hotAlbumList, albumList, page, total} = this.state;
  	let hotAlbums;
    let albums;
...
class AlbumItem extends Component {
  constructor(props) {
    super(props);
    // 切换播放列表
    this.changePlayMusic = this.changePlayMusic.bind(this);
  }
	changePlayMusic(id){
    ...
    return (
      <li>
        <div className="u-cover u-cover-alb2">
          ...
          <a onClick={() => this.changePlayMusic(album.id)} href="javascript:;" className="icon-play f-fr" />
        </div>
        <p className="dec">
