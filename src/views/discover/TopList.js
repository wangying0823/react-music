...
class TopList extends Component{
	constructor(props){
      ...
      globalList: [],//全球榜
      currId: 19723756,
      listDetail: null,//list详情
      commentData: null,// 评论内容
		}
    ...
    api.getTops().then(res => {
      if (res.data.code === 200) {
        const featureList = res.data.list.filter(i => i.ToplistType); // 特色榜
        const globalList = res.data.list.filter(i => !i.ToplistType); // 全球榜
        this.setState({
          featureList,
          globalList,
        });
